...
# Known limitations, using a directory input with a space in it
...
$LINUX_PATH = "/mnt/{0}" -f ($DIRECTORY_LOCATION -replace "\\", "/" -replace ":", "")
...
# Run Linux command 
C:\Windows\System32\bash.exe -l -c "find $LINUX_PATH -type d | sed 's:[^-][^/]*/:--:g; s:^-: |:'"
...
# Run Linux command 
C:\Windows\System32\bash.exe -l -c "find $LINUX_PATH | sed 's:[^-][^/]*/:--:g; s:^-: |:'"
